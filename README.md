...
`recording.postProcess.commands` は実行ファイルの `command` と argv 配列の `arguments` を分けて指定します。シェルを介さないため、番組名などに含まれる空白や記号がコマンドとして解釈されません。複数のコマンドは記載順に実行され、いずれかが失敗またはタイムアウトすると後続のコマンドは実行しません。完全な番組 JSON は標準入力から渡され、引数内では `{recordedPath}`、`{programID}`、`{title}`、`{channelName}`、`{channelType}`、`{channelNumber}`、`{startAtUnixMilli}`、`{endAtUnixMilli}` を置換できます。

```json
"postProcess": {
  "commands": [{
    "command": "/usr/local/bin/archive-recording",
    "arguments": ["{recordedPath}", "{programID}"]
  }],
  "timeoutSeconds": 600,
  "maxConcurrentRuns": 1
}
...
`commands` が空または未指定の場合は実行しません。`timeoutSeconds` は各コマンドに適用され、0 の場合は 10 分です。`maxConcurrentRuns` が 0 の場合は同時に 1 件まで実行します。コマンドには録画ファイルの移動・削除権限を与えないことを推奨します。
...
- `log/`: 1 行 1 レコードの構造化ログ（`timestamp|level=...|event=...|key=value`）。時刻はOSのローカルタイムゾーンを使用します。各ログは10 MiBでローテーションし、`<log>.1` ～ `<log>.5` の5世代を保持します。
