...
`recording.postProcess.command` はシェル文字列ではなく argv 配列です。シェルを介さないため、番組名などに含まれる空白や記号がコマンドとして解釈されません。完全な番組 JSON は標準入力から渡され、引数内では `{recordedPath}`、`{programID}`、`{title}`、`{channelName}`、`{channelType}`、`{channelNumber}`、`{startAtUnixMilli}`、`{endAtUnixMilli}` を置換できます。

```json
"postProcess": {
  "command": ["/usr/local/bin/archive-recording", "{recordedPath}", "{programID}"],
  "timeoutSeconds": 600,
  "maxConcurrentRuns": 1
}
```

`command` が空または未指定の場合は実行しません。`timeoutSeconds` が 0 の場合は 10 分、`maxConcurrentRuns` が 0 の場合は同時に 1 件まで実行します。コマンドには録画ファイルの移動・削除権限を与えないことを推奨します。
