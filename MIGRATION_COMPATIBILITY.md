...
| `wuiOpenServer`, `wuiOpenHost`, `wuiOpenPort` | Unauthenticated LAN listener. | partially compatible; starts a separate HTTP server without Basic auth and selects a private IPv4 when `wuiOpenHost` is unset. |
...
| `wuiMdnsAdvertisement` | mDNS advertisement. | intentionally changed; the existing config field is parsed explicitly and `compat check` warns when set, but mDNS service advertisement is not implemented to avoid an extra runtime dependency for personal deployments. |
...
The old WUI serves `web/` directly with static files, range support, cache headers for icons/images, fixed extension-based content types, Host-header validation, the Socket.IO client script, and API dispatch under `/api/`. The Go implementation serves static files from `web/` when present and can fall back to the sibling legacy `web/` directory during development. The repository now includes a dependency-free native Strata PVR dashboard at `web/index.html`, `web/app.js`, and `web/styles.css`; it reads status, reserves, recording, recorded, schedule, rules, logs, and selected non-secret config summaries from the Go API and exposes basic actions for schedule reservation, reserve skip/unskip/manual removal, active recording stop, recorded watch/download/delete, and rule enable/disable/delete/add-from-JSON. The native rule form covers common fields and can merge an extra JSON object for less common rule extensions while keeping the full JSON editor available for exact edits. Static `.ico` and `.png` assets now preserve the legacy `Cache-Control: private, max-age=86400` behavior while other static assets keep `no-cache`; legacy content types for html/js/css/icons/images/video/json/xspf are set explicitly; common legacy security headers are preserved and the `Server` header now identifies `Strata PVR`; static `Last-Modified`/`If-Modified-Since` conditional requests return 304 like the old WUI; missing static files and out-of-range static Range requests return the old fixed text/plain 404/416 bodies; `/socket.io/socket.io.js` returns a lightweight no-Node compatibility client that fires connect/status/notify callbacks, polls status every 5 seconds plus data notifications every 15 seconds, and clears polling timers on `disconnect()` so existing WUI assets can load and refresh API data. Full Socket.IO realtime push remains incomplete; requests without `Host` return 400. Current status: partially compatible; Node-based frontend builds are not required.

Frontend rewrite task: partially compatible. The native dashboard, core reservation/recording/recorded actions, recorded M2TS/direct-MP4/720p-MP4/low-bitrate-MP4/XSPF/download actions, active-recording M2TS watch action, JSON-based rule actions including in-place JSON updates, common title/description/type/SID/category/channel/flag/duration/hour/recorded-format rule creation, extra JSON rule-field merging, log panels, read-only settings summary, and basic schedule navigation by channel/time range/item count are implemented. Native settings editing is intentionally read-only for personal deployments because direct `config.json` edits are easier to review and avoid accidental runtime reconfiguration; the legacy-compatible `/api/config.json` PUT endpoint remains available for old WUI/API clients. Custom playback/transcode controls still need native replacements before the legacy WUI fallback can be removed.
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, recorded playback/download/transcode-preset actions, JSON-based create/update, expanded common form rule actions, extra JSON rule-field merging, log panels, intentionally read-only settings summary, and basic schedule navigation added under `web/`, but full legacy WUI workflow coverage is incomplete. |
...
| Compat doctor/check/diff/backup/wrapper | implemented; validates required JSON state files and expected object/array shapes, `data/`, writable `log/`, writable `recordedDir`, native Strata PVR or legacy WUI static entry files, available disk space lookup, `ffmpeg`/`ffprobe` command availability, Mirakurun services/programs/tuners reachability, Node.js runtime non-requirement, warns about intentionally omitted personal-use-overkill integrations, prints non-secret config summaries, state-count summaries, active-recording migration warnings, and local wrapper-target binary warnings from `compat doctor`, reports dry-run JSON rewrite differences for compatible state files, can back up current JSON state files under `backup/strata-pvr-*`, and can print a safe shell wrapper for manual review/install. |
