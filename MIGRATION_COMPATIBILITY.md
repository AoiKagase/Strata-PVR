...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, recorded playback/download/transcode-preset/custom playback actions, JSON-based create/update, expanded common form rule actions, extra JSON rule-field merging, log panels, intentionally read-only settings summary, and basic schedule navigation added under `web/`, but full legacy WUI workflow coverage is incomplete. |
