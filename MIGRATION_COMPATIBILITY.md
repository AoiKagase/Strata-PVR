...
- Legacy UI edge cases: native dialogs now cover focus restoration, backdrop close, pending MP4 state cleanup, and rule-creation navigation cleanup, but unusual old-WUI affordances, exact dialog behavior, and every small legacy navigation shortcut have not been exhaustively ported.
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, recorded-file M2TS playback/download, MP4 conversion playback actions, dashboard/current-channel and channel-program navigation, reservation/recorded page search and category filtering, program detail/channel playback/rule/manual-reserve flows, JSON-based create/update, expanded common form rule actions, extra JSON rule-field merging, log panels, editable common/detailed config form plus raw JSON editor, and expanded schedule navigation added under `web/`; live and recording-in-progress UI now avoids M2TS buttons and uses playback-oriented labels instead of exposing MP4 as the user-facing action name. Remaining work is realtime Socket.IO push, rare non-retired config/rule field-specific controls where they have runtime value, exhaustive old-WUI edge affordances such as minor shortcuts/dialog parity/state restoration, and browser verification of the frontend flows. Unknown/custom fields and retired Twitter/Tweeter fields intentionally stay in raw JSON rather than dedicated forms. |
