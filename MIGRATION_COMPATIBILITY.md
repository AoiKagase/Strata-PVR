...
| `compat check`, `compat doctor`, `compat diff`, `compat backup`, `compat wrapper` | implemented | New Go-only safety checks for required JSON state files, `data/`, writable `recordedDir`, WUI static assets, available disk space lookup, Mirakurun services/programs/tuners reachability, Node.js runtime non-requirement, warnings for intentionally omitted personal-use-overkill integrations, dry-run JSON rewrite difference reporting, timestamped JSON state backups under `backup/`, plus a safe shell wrapper generator that prints to stdout and never overwrites existing files. |
...
Frontend rewrite task: partially compatible. The native dashboard, core reservation/recording/recorded actions, JSON-based rule actions, log panels, read-only settings summary, and basic schedule navigation by channel/time range/item count are implemented. Settings editing, richer playback controls, and a friendlier rule form still need native replacements before the legacy WUI fallback can be removed. For personal deployments, settings editing is a candidate for intentional read-only behavior because direct `config.json` edits are easier to review and avoid accidental runtime reconfiguration.
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, JSON-based rule actions, log panels, read-only settings summary, and basic schedule navigation added under `web/`, but full legacy WUI workflow coverage is incomplete. |
