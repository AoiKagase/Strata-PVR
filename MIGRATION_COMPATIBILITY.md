...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, recorded playback/download/transcode-preset/custom playback actions, dashboard/current-channel and channel-program navigation, program detail/channel MP4/rule/manual-reserve flows, JSON-based create/update, expanded common form rule actions, extra JSON rule-field merging, log panels, editable common/detailed config form plus raw JSON editor, and expanded schedule navigation added under `web/`; remaining work is realtime Socket.IO push, field-specific coverage for rare non-retired config/rule extensions, exhaustive old-WUI edge affordances, and browser verification of the frontend flows. |
