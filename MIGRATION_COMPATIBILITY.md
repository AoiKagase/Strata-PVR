...
| `service <operator|wui> <initscript|execute>` | partially compatible | Initscript generation uses the Go binary and includes legacy LSB headers, start/stop/restart/status handling, `/var/run/chinachu-*.pid`, `su $USER` launch, and process-group `SIGQUIT` stop behavior. `operator execute` runs the Go operator loop; `wui execute` starts the Go WUI/API server. |
