...
Frontend rewrite task: partially compatible. The native dashboard, core reservation/recording/recorded actions, recorded M2TS/direct-MP4/720p-MP4/low-bitrate-MP4/XSPF/download actions, active-recording M2TS watch action, JSON-based rule actions including in-place JSON updates, common title/description/type/SID/category/channel/flag/duration/hour/recorded-format rule creation, log panels, read-only settings summary, and basic schedule navigation by channel/time range/item count are implemented. Native settings editing is intentionally read-only for personal deployments because direct `config.json` edits are easier to review and avoid accidental runtime reconfiguration; the legacy-compatible `/api/config.json` PUT endpoint remains available for old WUI/API clients. Custom playback/transcode controls and friendlier form editing for arbitrary JSON extensions and less common rule fields still need native replacements before the legacy WUI fallback can be removed.
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, recorded playback/download/transcode-preset actions, JSON-based create/update and expanded common form rule actions, log panels, intentionally read-only settings summary, and basic schedule navigation added under `web/`, but full legacy WUI workflow coverage is incomplete. |
