...
| `storageLowSpaceThresholdMB`, `storageLowSpaceAction`, `storageLowSpaceNotifyTo`, `storageLowSpaceCommand` | Low disk behavior. | partially compatible; `remove`, `stop`, hook command, legacy Chinachu sendmail notification text, and three-hour notification throttling are implemented. |
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, recorded-file M2TS playback/download/start-offset playback, MP4 conversion playback actions, dashboard/current-channel and channel-program navigation, reservation/recorded/channel-program/rule search, category/state filtering, sort controls, clear actions, Esc-to-clear search shortcuts, and local restoration, program detail/channel playback/rule/manual-reserve flows, schedule/on-air/detail reservation and recording state badges with active recording actions, JSON-based create/update, expanded common form rule actions, extra JSON rule-field merging, log panels, editable known-runtime config form plus raw JSON editor, destructive/config/rule add-save confirmation dialogs with metadata where relevant, focus restoration, OK-button initial focus for Enter-key confirmation, and Tab/Shift+Tab focus wrapping inside native dialogs, initial loading/failure placeholders with refresh-button busy state, same-origin realtime notifications after mutating API calls, keyboard shortcuts for navigation/refresh/search/list movement including search-filter clear, line/page/first/last jumps/dialog close/detail opening, selected-program highlighting, scroll restoration, and expanded schedule navigation added under `web/`; live and recording-in-progress UI now avoids M2TS buttons and uses playback-oriented labels instead of exposing MP4 as the user-facing action name. After the `..\Chinachu` source audit, no known functionally required WUI/API runtime gap remains. Rare future config/rule field-specific controls, exhaustive old-WUI edge affordances such as named presets/minor shortcuts/dialog parity/state restoration, third-party Socket.IO oracle checks, and browser verification remain parity/verification backlog. Unknown/custom fields and retired Twitter/Tweeter fields intentionally stay in raw JSON rather than dedicated forms. |
