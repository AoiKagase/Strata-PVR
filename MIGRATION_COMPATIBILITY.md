...
- Settings forms: common and operational fields are covered, but non-retired secret fields and deeply nested or rare legacy config objects still require the raw JSON editor. Retired Tweeter/Twitter fields should stay out of the native form and remain config-preserved only.
- Rule forms: common rule predicates and extra JSON merging are implemented, but the old WUI's full field-specific editing surface for every uncommon rule extension is not complete.
