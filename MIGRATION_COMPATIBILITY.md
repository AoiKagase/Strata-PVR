...
| `compat check`, `compat doctor`, `compat diff`, `compat backup` | implemented | New Go-only safety checks for required JSON state files, `data/`, writable `recordedDir`, legacy WUI static entry files/directories, available disk space lookup, Mirakurun services/programs/tuners reachability, Node.js runtime non-requirement, warnings for intentionally omitted personal-use-overkill integrations, dry-run JSON rewrite difference reporting, plus timestamped JSON state backups under `backup/`; does not alter legacy command behavior. |
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard and core reservation/recording/recorded actions added under `web/`, but full legacy WUI workflow coverage is incomplete. |
...
| Compat doctor/check/diff/backup | implemented; validates required JSON state files, `data/`, writable `recordedDir`, native Strata PVR or legacy WUI static entry files, available disk space lookup, Mirakurun services/programs/tuners reachability, Node.js runtime non-requirement, warns about intentionally omitted personal-use-overkill integrations, reports dry-run JSON rewrite differences for compatible state files, and can back up current JSON state files under `backup/strata-pvr-*`. |
