...
Frontend rewrite task: partially compatible. The native dashboard, core reservation/recording/recorded actions, JSON-based rule actions, log panels, and read-only settings summary are implemented, but detailed schedule navigation, settings editing, richer playback controls, and a friendlier rule form still need native replacements before the legacy WUI fallback can be removed.
...
| Native Strata PVR frontend rewrite | partially compatible; dependency-free dashboard, core reservation/recording/recorded actions, JSON-based rule actions, log panels, and read-only settings summary added under `web/`, but full legacy WUI workflow coverage is incomplete. |
