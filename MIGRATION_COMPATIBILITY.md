...
Frontend rewrite task: partially compatible. The native dashboard, core reservation/recording/recorded actions, recorded M2TS/direct-MP4/720p-MP4/low-bitrate-MP4/XSPF/download/preview actions, recorded custom start/length/quality playback controls, active-recording M2TS/MP4/XSPF/preview actions, selected-channel M2TS/MP4/XSPF actions, direct schedule channel-logo display, scheduler force-run, storage summary, config JSON reload/save with validation and confirmation, JSON-based rule actions including in-place JSON updates, common title/description/type/SID/category/channel/flag/duration/hour/recorded-format rule creation, extra JSON rule-field merging, log panels, settings summary, and schedule navigation by type/channel/day/genre/hidden-channel/time range/item count are implemented. Remaining frontend gaps are full legacy workflow parity, realtime Socket.IO push, field-specific native config forms beyond the raw JSON editor, and edge-case legacy UI affordances. The legacy-compatible `/api/config.json` PUT endpoint remains available for old WUI/API clients and is used by the native config editor.
