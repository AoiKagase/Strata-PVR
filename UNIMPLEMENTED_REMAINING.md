...
| `program/watch.js`, `channel/watch.js` | MP4/M2TS/XSPF watch forms, in-browser playback where possible, VLC deep links on mobile, local playback settings, volume control. | API watch routes, native playback/XSPF flows, and mobile-only VLC intent/callback shortcuts are implemented. Desktop external playback is intentionally left to normal MP4/M2TS/XSPF links and OS/browser associations rather than VLC-specific links. Exact old watch form/settings and inline toolbar behavior remain parity backlog. |
...
| Legacy context-menu utilities | partial / intentionally limited | Old grids and schedule cards exposed tweet, SCOT copy, ID/title/description copy, Google, related-site, and Wikipedia actions. Native WUI implements the useful copy subset from program details: program ID, title, and description. Tweet, SCOT, related-site, Google, Wikipedia, and old right-click menu reproduction remain intentionally omitted because they do not change recorder state and are external/legacy convenience actions. |
| Legacy program detail extras | partial | Core program metadata and actions are available. Native WUI shows one preview image in program details for recording and recorded programs, alerts when a recorded preview reports the file as missing, and marks recorded list rows whose preview cannot be loaded for file confirmation. Old URL auto-linking, three-position thumbnail gallery, previous/next pager, and Left/Right pager shortcuts are not fully reproduced. |
| Legacy watch-page browser integrations | partial | Playback, MP4 conversion options, XSPF, recorded-file download/start-offset workflows, and mobile-only VLC intent/callback launching are covered. PC playback intentionally uses normal MP4/M2TS/XSPF links so OS/browser associations can choose the player. Local `channel.watch.settings` parity and exact inline video toolbar behavior are not implemented. |
