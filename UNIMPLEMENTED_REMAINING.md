...
| WUI frontend | Legacy list/filter affordances | partial | Main dashboard, schedule, recording, recorded, rule, and manual reserve flows are implemented. Fine legacy affordances such as every old sort/filter shortcut, compact page transition, and historical list interaction remain unverified. |
| WUI frontend | Legacy dialogs and confirmations | partial | Core JSON-backed create/update/delete dialogs exist, but exact old dialog text, focus order, default button behavior, cancel/close behavior, and validation timing are not byte-for-byte compatible. |
