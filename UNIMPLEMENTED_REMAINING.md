...
| Legacy program detail extras | partial | Core program metadata and actions are available. Native WUI now shows one preview image in program details for recording and recorded programs. Old URL auto-linking, recorded file-size alert, three-position thumbnail gallery, previous/next pager, and Left/Right pager shortcuts are not fully reproduced. |
