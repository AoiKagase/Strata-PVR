...
| Legacy program detail extras | partial | Core program metadata and actions are available. Native WUI shows one preview image in program details for recording and recorded programs, and alerts when a recorded preview reports the file as missing. Old URL auto-linking, three-position thumbnail gallery, previous/next pager, and Left/Right pager shortcuts are not fully reproduced. |
