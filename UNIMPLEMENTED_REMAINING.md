...
| WUI frontend | Legacy dialogs and confirmations | partial | Native confirmation dialog now covers destructive actions, config save, and rule add/save from both the JSON editor and form editor, with metadata where relevant, cancel/backdrop/Esc handling, danger styling, focus restoration, and OK-button initial focus for Enter-key confirmation. Exact old dialog text, browser-specific default-button edge cases, and validation timing are still not byte-for-byte compatible. |
